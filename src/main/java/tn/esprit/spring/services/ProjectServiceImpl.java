...
//	@Scheduled(cron = "*/5 * * * * *" )
	...
	public void EnvoyerRapport(HttpServletResponse response) {
		...
		for (Project project: retrieveAllProjects())
		{	int id=project.getIdProject();
		if(projectRepository.getEtatOfProject(id)==Etat.done)
		{
			Project pp= projectRepository.findById(id).orElse(null);
           List<Integer> idEntreprises = projectRepository.getEntrepriseEtatProjetDone(id);
	...
	//	Employee employee = employeeRepository.findById(idEmployee).orElse(null);
        ...
        document.open();
        Font fontTitle = FontFactory.getFont(FontFactory.HELVETICA_BOLD, Font.DEFAULTSIZE, Font.UNDERLINE, new Color(0xFF, 0x00, 0x00));
        ...
        Font blue = FontFactory.getFont(FontFactory.HELVETICA, Font.DEFAULTSIZE, Font.BOLD, new Color(0x00, 0x00, 0xFF));
        Font green = FontFactory.getFont(FontFactory.HELVETICA, Font.DEFAULTSIZE, Font.ITALIC, new Color(0x00, 0x80, 0x00));
        Font  yellow= FontFactory.getFont(FontFactory.HELVETICA, Font.DEFAULTSIZE, Font.ITALIC, new Color(0xFF, 0xFF, 0x00));
        Font  titre= FontFactory.getFont(FontFactory.HELVETICA, Font.DEFAULTSIZE, Font.UNDERLINE, new Color(0x69, 0x69, 0x69));
       ...
       // paragraph1.add(new Chunk("red", red));
        ...
        Paragraph paragraph10 = new Paragraph("Nombre de task en retard: "+p.getNbreTaskLate(), fontParagraph);
